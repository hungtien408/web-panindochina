...
public partial class ad_single_downloadcategory : System.Web.UI.Page
{
    ...
    protected void RadGrid1_ItemDataBound(object sender, GridItemEventArgs e)
    {
        if (e.Item is GridEditableItem && e.Item.IsInEditMode)
        {
            ...
            var FileImageName = (RadUpload)row.FindControl("FileImageName");
            var DownloadCategoryID = ((HiddenField)row.FindControl("hdnDownloadCategoryID")).Value;
            var dv = (DataView)ObjectDataSource1.Select();
            var ddlParent = (RadComboBox)row.FindControl("ddlParent");

            if (!string.IsNullOrEmpty(DownloadCategoryID))
            {
                dv.RowFilter = "DownloadCategoryID = " + DownloadCategoryID;
