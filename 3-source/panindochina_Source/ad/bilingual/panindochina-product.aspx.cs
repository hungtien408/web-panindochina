...
public partial class ad_single_product : System.Web.UI.Page
{
    ...
    protected void RadGrid1_ItemCommand(object sender, GridCommandEventArgs e)
    {
        ...
        {
            string ProductID, Priority, PriorityByProduct, PriorityByProductCat, InStock, IsHot, IsNew, IsBestSeller, IsSaleOff, IsAutomotive, IsSteelMills, IsMining, IsConstruction,
            ...
            foreach (GridDataItem item in RadGrid1.Items)
            {
                ...
                PriorityByProductCat = ((RadNumericTextBox)item.FindControl("txtPriorityByProductCat")).Text.Trim();
        ...
        else if (e.CommandName == "PerformInsert" || e.CommandName == "Update")
        {
            ...
            string PriorityByProductCat = ((RadNumericTextBox)row.FindControl("txtPriorityByProductCat")).Text.Trim();
            ...
            string ManufacturerProduct = ((RadTextBox)row.FindControl("txtManufacturer")).Text.Trim();
            string ModelProduct = ((RadTextBox)row.FindControl("txtModel")).Text.Trim();
            string Specifications = HttpUtility.HtmlDecode(FCKEditorFix.Fix(((RadEditor)row.FindControl("txtSpecifications")).Content.Trim()));
            string SpecificationsEn = HttpUtility.HtmlDecode(FCKEditorFix.Fix(((RadEditor)row.FindControl("txtSpecificationsEn")).Content.Trim()));
