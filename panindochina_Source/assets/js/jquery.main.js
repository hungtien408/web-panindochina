...
function myfunload() {
    ...
    if ($("#controlTab").size() == 1) {
        tabrpoduct();
        if ($(".jcarousel-protab").size() > 0) {
            $(".jcarousel-protab").each(function () {
                var countli = $(this).find(".jcarousel").find("li").size();
                if (countli == 0) {
                    $(this).remove();
                }
            });
            $(".jcarousel-protab").jcarouselbox({
                autos: false,
                pausehover: false,
    ...
    if ($("#jcarouselDetails").size() == 1) {
        $("#jcarouselDetails").jcarouselzoom({
            ...
            shownum: 3, // show number thums
            usezoom: true
        });
    }

    $(".tabs-details .nav-tabs li:first").addClass("active");
    $(".tabs-details .tab-content .tab-pane:first").addClass("active");
    if ($('.list-box').size() > 0) {
        $('.list-box').isotope({
            itemSelector: 'li',
            layoutMode: 'fitRows'
...
function myListTb() {
    ...
    if ($('.product-tb').size() > 0) {
        $('.product-tb').textHeight({
            activetit: true,
            listcss: [{ cssname: ".product-img" }, { cssname: ".product-name" }, { cssname: ".description" }],
            wpointb: false,
            widthpont: 479,
            desbool: false,
            listpos: [{ cssnamepos: ".description", cssheightnum: "3" }],
        ...
        $("#myModal .modal-body").load(pagev, function (responseTxt, statusTxt, xhr) {
            if (statusTxt == "success") {
                if ($("#jwplayer").size() == 1) {
                    ...
                    $("#myModal .modal-body").html('<div class="uptext">Đang cập nhật</div>');
                }
            }
            if (statusTxt == "error") {
    ...
    $(".box-gallery a").click(function (e) {
        ...
        $("#myModal .modal-body").load(pagev, function (responseTxt, statusTxt, xhr) {
            if (statusTxt == "success") {
                var countb = $("#jcarouselGallery .carousel-stage li").size();
                if (countb > 0) {
                    ...
                    $("#myModal .modal-body").text("Đang cập nhật...");
                }
            }
            if (statusTxt == "error") {
                alert("Error: " + xhr.status + ": " + xhr.statusText);
